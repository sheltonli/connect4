...
While you are playing over the network, please open the console and wait until it says "It's my turn!" cause otherwise it will mess up.

When a player wins their status is updated in the database. When a player ties it is updated in the database.
This is not currently reflected in the ui in anyway.

The amazingly awesome board we made using THREE.js
To make this work each column is clickable.  The pieces are only added when it makes sense (ie. when there is space for them in the column).
...
I had some previous experience with THREE.js and thought it would be cool.
The Captcha was implemented for users when registering.
Our site has been checked on CDF.  Thanks for your time in marking it!
